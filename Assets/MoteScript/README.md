...
`&&`と`||`は短絡評価されます。`&&`は左辺が偽の場合、`||`は左辺が真の場合に右辺を評価しません。そのため、右辺に関数呼び出しや代入が含まれていても、短絡した場合はその副作用は発生しません。

```text
callCount=0;0 && (callCount=callCount+1);callCount
```

この例では`&&`の左辺が偽なので右辺の代入は実行されず、結果は`0`です。
