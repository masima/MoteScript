...
namespace MoteScript.Tests
{
	public partial class MoteScriptTests
	{
		...
		{
			Assert.AreEqual(EValueType.Unknown, _decoder.Decode(string.Empty).Evaluate(null).ValueType);
			Assert.AreEqual(0, _decoder.Decode("array=();array").Evaluate(new Context()).GetArray().Count);
			Assert.AreEqual(0, _decoder.Decode("dictionary=[];dictionary").Evaluate(new Context()).GetDictionary().Count);
		}
		...
		public void TestBoundary_LargeArrayLiteral()
		{
			...
			MoteList<float> result = _decoder.Decode(sentence).Evaluate(new Context()).GetArray();
		...
		public void TestBoundary_LargeDictionaryLiteral()
		{
			...
			IContext<float> result = _decoder.Decode($"dictionary=[{entries}];dictionary")
				.Evaluate(new Context()).GetDictionary();
		...
		public void TestBoundary_DeepParentheses()
		{
			...
			Assert.AreEqual(3, _decoder.Decode(sentence).Evaluate(null).IntegerValue);
		}
		...
		public void TestBoundary_DeepIfBlocks()
		{
			...
			Assert.AreEqual(1, _decoder.Decode(sentence).Evaluate(new Context()).IntegerValue);
		}
		...
		public void TestBoundary_LongDictionaryPath()
		{
			...
			_decoder.Decode($"{joinedPath}=123").Evaluate(context);
		...
		{
			Assert.AreEqual(1,
				_decoder.Decode("constant=()=>{1};constant()").Evaluate(new Context()).IntegerValue);
			...
			Assert.AreEqual(36, _decoder.Decode(sentence).Evaluate(new Context()).IntegerValue);
		}
	}
}
